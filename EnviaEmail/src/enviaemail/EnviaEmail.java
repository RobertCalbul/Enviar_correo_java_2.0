...
public class EnviaEmail {
    ...
    public Boolean enviar() {
        ...
        if (identifica.equals("gmail")) {                   //#si el correo es de tipo gmail
        ...
        } else {//#de lo contrario supondremos que es hotmail
        ...
        try {
            ...
            if (adjunto != null) { //#Si biene algun archivo adjunto
                for (Archivo_Class adjunto1 : adjunto) {//Recorre la lista de archivos
                    BodyPart adj = new MimeBodyPart();//#Instancea un parte del mensaje
                    adj.setDataHandler(
                            new DataHandler(
                                    new FileDataSource(adjunto1.ruta)
                            ));                         //#Setea un nuevo adjunto al correo
            ...
            //#FIN AGREGAR MULTIPLES ADJUNTOS
            
            //#INICIO AGREGAR MULTIPLES DESTINATARIOS
            for (int i = 0; i < this.destino.get(0).size(); i++) {//#Recorre lista de destinos
                msg.addRecipient(Message.RecipientType.TO,
                        new InternetAddress(
                                this.destino.get(0).get(i)
                        ));//#agrega el destinatario 
            }//#Fin for
            //#FIN AGREGAR MULTIPLES DESTINATARIOS
            
            //#INICIO AGREGAR MULTIPLES CC
            if (this.destino.get(1) != null) {//#Si existen destinatarios con copia
                for (int i = 0; i < this.destino.get(1).size(); i++) {//#Recorre la lista del segundo indice
            ...
            if (this.destino.get(2) != null) {//Si existen adestinatarios con copia oculta
                for (int i = 0; i < this.destino.get(2).size(); i++) {//#Recorre lista
